...
import { getMovementReport } from '../report-utils/services/ReportFunctions';
...
const MovementReport = () => {
  ...
  const handleMapView = row => {
    ...
    let {
      ...
      movementEnd,
      movementDuration,
      movementLatitude: lat,
      movementLongitude: lng
    } = row;
    let info = [
      `Start Date : ${startDate}`,
      `End Date : ${endDate}`,
      `Movement Start Time : ${movementStart}`,
      `Movement End Time : ${movementEnd}`,
  ...
  // function to get movement report data
  const onSubmit = async data => {
    ...
    let {
      ...
      pdfURL,
      excelURL
    } = await getMovementReport(payload);
    columns = [
      ...columns,
      {
        title: 'Actions',
